...
<Callout type="info">
本文假设你已了解 [Rules 的工作原理](./how-rules-work)，包括 Rules 类型、应用方式和文件结构。
</Callout>
...
在 `.cursor/rules/` 目录下按主题拆分 Rules 文件，使用 `globs` 限定作用域。关于 Rules 的四种应用方式，请参考 [Rules 应用方式](./how-rules-work#rules-应用方式)。
...
关于 `.mdc` 文件的完整格式规范，请参考 [Rules 文件结构](./how-rules-work#rules-文件结构)。一个完整的 Rules 文件通常包含以下部分：
...
了解了编写原则后，接下来我们看看[在开发过程中何时编写这些 Rules](./development-sequence)。
