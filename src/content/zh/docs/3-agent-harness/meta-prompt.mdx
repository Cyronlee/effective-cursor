...
这套流程同样可用于生成 **AGENTS.md 骨架**：只需把模板里的「规则」替换为「项目契约」（技术栈、build/test 命令、目录职责、do-not-touch 路径），输出即可作为跨工具的项目文档。
...
- [Cursor Rules 最佳实践](/zh/docs/3-agent-harness/best-practices) - 规则编写的核心原则
- [Rules 编写实战顺序](/zh/docs/3-agent-harness/development-sequence) - 何时编写什么规则
- [AGENTS.md 跨工具标准](/zh/docs/3-agent-harness/agents-md) - 用同一模板生成项目契约
- [Vite + Shadcn/UI 规则集](/zh/rules/vite-shadcnui) - 前端完整规则示例
- [Spring Boot 规则集](/zh/rules/springboot) - 后端完整规则示例

## 下一步

完成 Rules 的编写后，接下来我们将进入[第四章：规范工作流](/zh/docs/4-workflow)，学习如何将这些规则融入日常开发流程。
