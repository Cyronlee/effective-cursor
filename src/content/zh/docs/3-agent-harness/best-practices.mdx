...
本章的编写原则属于 **harness configuration 层**：它们对 AGENTS.md 与 .mdc 两类载体通用，既适用于 Cursor 的 Project Rules，也适用于跨工具的项目契约。关于载体选择见「AGENTS.md 跨工具标准」，Rules 基础见「Rules 工作原理」。

<Callout type="info">
本文假设你已了解 [Rules 的工作原理](/zh/docs/3-agent-harness/how-rules-work)，包括 Rules 类型、应用方式和文件结构。
</Callout>
...
[awesome-cursorrules](https://github.com/PatrickJS/awesome-cursorrules) 是 GitHub 上的热门项目，但其中大量 Rules 都是废话。例如它的 [react.mdc](https://github.com/PatrickJS/awesome-cursorrules/blob/main/rules-new/react.mdc)：
...
| [3. 分文件匹配](#split-files) | 按主题拆分，用 globs 限定作用域 | 所有 Rules 塞在一个文件 | api-rules.mdc 只管 src/clients/** |
| [4. 提供示例](#examples) | 附上 import 和代码示例 | "使用 SWR 获取数据" | 完整的 useSWR 代码片段 |
| [5. 快速迭代](#iterate) | 从 3-5 条痛点 Rules 开始 | 一次性写 500 行"完美 Rules" | 发现问题立即补充 |
...
在 `.cursor/rules/` 目录下按主题拆分 Rules 文件，使用 `globs` 限定作用域。关于 Rules 的四种应用方式，请参考 [Rules 应用方式](/zh/docs/3-agent-harness/how-rules-work#rules-应用方式)。
...
关于 `.mdc` 文件的完整格式规范，请参考 [Rules 文件结构](/zh/docs/3-agent-harness/how-rules-work#rules-文件结构)。一个完整的 Rules 文件通常包含以下部分：
...
- [全局 Rules 示例](/zh/rules/vite-shadcnui/global-rules) - 技术栈、目录结构、TypeScript 规范
- [API Rules 示例](/zh/rules/vite-shadcnui/api-rules) - API 客户端架构、SWR 集成
- [路由 Rules 示例](/zh/rules/vite-shadcnui/routing-rules) - 页面结构、路由保护
- [UI Rules 示例](/zh/rules/vite-shadcnui/ui-rules) - 组件使用、图标系统

## 下一步

了解了编写原则后，接下来我们看看[在开发过程中何时编写这些 Rules](/zh/docs/3-agent-harness/development-sequence)，以及如何把项目级契约放进 [AGENTS.md](/zh/docs/3-agent-harness/agents-md)。
