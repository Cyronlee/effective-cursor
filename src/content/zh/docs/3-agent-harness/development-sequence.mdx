...
AGENTS.md 是项目的「生根文件」：脚手架一完成，第一版就要建立（技术栈、build/test 命令、目录职责），之后随开发阶段按需扩充。它从第一天起就是跨工具的稳定锚点，后续每阶段的沉淀都可以先落进 AGENTS.md，再按需拆分成 path-specific 的 Rules。
...
有时候你可能需要从零开始为一个新的技术栈编写 Rules。接下来我们将分享[用于生成 Rules 的元提示词](/zh/docs/3-agent-harness/meta-prompt)，帮助你快速生成 Rules 框架；需要建立项目级契约时，也可以参考 [AGENTS.md 跨工具标准](/zh/docs/3-agent-harness/agents-md)。
