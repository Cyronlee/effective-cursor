---
title: "Rules and Conventions Across Development Phases"
---

import { Callout } from 'nextra/components'

# Rules and Conventions Across Development Phases

> Write the right rules at the right time

## Core Philosophy

Project conventions are not something you write once and forget. They are a **system of standards that accumulates as your project evolves**.
...
<Callout type="info">
**AGENTS.md is the project's "root file".** Establish its first version right after scaffolding — stack, commands, do-not-touch paths — then extend it in the same phases below. This keeps the cross-tool contract in sync with harness evolution. See [AGENTS.md Cross-Tool Standard](/en/docs/3-agent-harness/agents-md).
</Callout>
...
When you finish project initialization and determine the basic tech stack and directory structure, immediately write **global rules** and the first `AGENTS.md`.
...
```markdown
# Example: Add immediately after project init
...
**Iteration principle: Problem → Rule → Verify → Optimize** — the same harness-engineering loop described in [Verify Closed Loop](/en/docs/3-agent-harness/verify-closed-loop).
...
This means you need to carefully design `globs` patterns so rules are **mutually exclusive or have low overlap**:
...
ui-rules.mdc        → globs: src/components/**
page-rules.mdc      → globs: src/pages/**
```
...
**Immediately create**: `AGENTS.md` (root file) + `global-rules.mdc`
...
Sometimes you may need to write rules from scratch for a new tech stack. Next, we'll share [meta prompts for generating Rules](/en/docs/3-agent-harness/meta-prompt) to help you quickly generate rule frameworks.
