...
flowchart TD
    ...
    subgraph Phase2["📚 Phase 2: Prepare Documents"]
        B --> C[Gather reference docs]
        C --> D[Design mockups/API docs/Sample code]
    end
    
    subgraph Phase3["📝 Phase 3: Solution Design"]
        D --> E[Write draft.md]
...
```
@prds/order-management/task-1-order-list/draft.md
@prds/order-management/shared/order-schema.sql
...
Output to @prds/order-management/task-1-order-list/final.md
```
...
```
@prds/order-management/task-1-order-list/final.md
...
```
@prds/comment/task-1-api/draft.md
...
```
@prds/comment/task-1-api/final.md
...
- Placed in `prds/` directory, organized by feature
